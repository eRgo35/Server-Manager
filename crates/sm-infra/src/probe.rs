...
impl StatusProbe for TcpStatusProbe {
    async fn is_up(&self, host: &str, port: u16, timeout: std::time::Duration) -> bool {
        matches!(
...
mod tests {
    ...
    async fn up_when_listener_present() {
        ...
        assert!(
            TcpStatusProbe
                .is_up("127.0.0.1", port, Duration::from_secs(1))
                .await
        );
    }
    ...
    async fn down_on_closed_port() {
        assert!(
            !TcpStatusProbe
                .is_up("127.0.0.1", 1, Duration::from_millis(200))
                .await
        );
    }
    ...
    async fn down_on_unroutable_within_timeout() {
        ...
        let up = TcpStatusProbe
            .is_up("10.255.255.1", 22, Duration::from_millis(300))
            .await;
        assert!(!up);
